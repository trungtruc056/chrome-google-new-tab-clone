...
describe('Component: SuggestList', () => {
    ...
    it('should render and active second item (Lao)', () => {
        const wrapper = mountWithTheme(<SuggestList data={mockData} activeIndex={1} />)

        expect(wrapper.find('li').at(1)).toHaveStyleRule('background-color', '#565656');
    });

    // simulate click item of suggestionList
    it('Simulate click', () => {
        const clickFn = jest.fn();
        const wrapper = mountWithTheme(<SuggestList data={mockData} onItemClicked={clickFn} />);
